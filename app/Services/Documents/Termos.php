...
class Termos extends BaseDocument
{
    ...
    {
        // Padrão não implementado para esta classe, use os métodos específicos
        return "";
    }

    public function gerarTermoCompromisso(Project $project): string
    {
        $this->templatePath = resource_path('templates/word/1. TERMO DE COMPROMISSO ÍGNEA.docx');
    ...
    {
        $this->templatePath = resource_path('templates/word/2. TERMO DE ENTREGA DE PROJETOS ÍGNEA.docx');
    ...
    {
        $this->templatePath = resource_path('templates/word/3. TERMO DE RESPONSABILIDADE DAS SAÍDAS DE EMERGÊNCIA.docx');
    ...
    {
        $this->templatePath = resource_path('templates/word/4. TERMO DE ENTREGA PARA SINDICO (para construtora entregar ao sindico futuramente).docx');
        return $this->salvar($this->getTemplateProcessor(), $project, 'Sindico');
    }

    public function gerarTermoInquilino(Project $project): string
    {
        $this->templatePath = resource_path('templates/word/4. TERMO DE ENTREGA PARA INQUILINO (para construtora entregar ao inquilino futuramente).docx');
