<x-app-layout>
    <x-slot name="header">
        <div class="precision-container pt-0 pb-0">
            <h2 class="precision-heading-3">
                {{ __('Editar Projeto: ') . $project->codigo_interno }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="precision-container pt-12" x-data="{
        ...
        form: {
            ...
            ocupacoes: {!! json_encode($project->ocupacoes ?? []) !!},
            medidas_selecionadas: {!! json_encode($project->medidas_selecionadas ?? []) !!},
            tem_glp: {{ $project->tem_glp ? 'true' : 'false' }},
            tem_gerador: {{ $project->tem_gerador ? 'true' : 'false' }},
            tem_subestacao: {{ $project->tem_subestacao ? 'true' : 'false' }},
            pne: {{ $project->pne ? 'true' : 'false' }},
        },
        addOcupacao() {
        ...
        <div class="precision-card max-w-4xl mx-auto">
            ...
            <div class="precision-card-header mb-8 border-none">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        "></h3>
                        <p class="precision-caption mt-1">Atualize os dados técnicos do projeto.</p>
                    </div>
            ...
            <form action="{{ route('projects.update', $project) }}" method="POST" x-ref="mainForm" class="space-y-6">
                @csrf
                @method('PATCH')
                ...
                <div x-show="step === 1" x-transition>
                    <div class="grid grid-cols-1 md:grid-cols-2 precision-gap-16">
                        <div>
                            ...
                            <input type="text" name="codigo_interno" x-model="form.codigo_interno" class="precision-input" required>
                        </div>
                        <div>
                            <label class="precision-label">Nome da Obra</label>
                            <input type="text" name="nome_obra" x-model="form.nome_obra" class="precision-input" required>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="cidade_analise_bombeiros" value="{{ $project->cidade_analise_bombeiros }}" class="precision-input">
                        </div>
                    </div>
                </div>
                ...
                <div x-show="step === 7" x-transition>
                    <div class="text-center py-8 space-y-6">
                        ...
                        <div>
                            <h4 class="precision-heading-3 mb-2">Atualização Pronta</h4>
                            <p class="precision-body text-gray-500">Verifique os dados e clique em atualizar para salvar as alterações do projeto.</p>
                        </div>
                        <div class="p-4 bg-blue-50 rounded-xl inline-flex items-center gap-3 border border-blue-100">
                            ...
                            <span class="text-sm font-semibold text-blue-900">Manter geração do Plano de Emergência ativa?</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="pt-6 mt-6 border-t border-gray-100 flex justify-between">
                    ...
                    <button type="button" x-show="step === totalSteps" @click="submit" class="precision-btn precision-btn-primary" style="background-color: var(--color-success);">Atualizar Projeto</button>
                </div>
